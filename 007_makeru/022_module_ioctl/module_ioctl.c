...
static int demo_open(struct inode *inodep, struct file * filep)  // 打开文件
{
	printk("%s,%d\n", __func__, __LINE__);
	return 0;
}

static int demo_release(struct inode *inodep, struct file * filep) // 关闭文件
{
	printk("%s,%d\n", __func__, __LINE__);
	return 0;
}
...
static ssize_t demo_read(struct file * filep, char __user * buffer, size_t size, loff_t * offlen)
{
	if(size > counter)
	{
		size = counter;
...
static ssize_t demo_write(struct file * filep, const char __user * buffer, size_t size, loff_t * offlen)
{
...
static long demo_ioctl(struct file * filep, unsigned int cmd, unsigned long arg)
{
	...
	case CMDR:
		...
		if(copy_to_user((struct node *)arg, &nodeinfo, sizeof(nodeinfo)))
		{
			printk("Failed to copy_to_user.\n");
			return -1;
		}
	...
	case CMDW:
		printk("CMDW OK!\n");
		if(copy_from_user(&nodeinfo, (struct node *)arg, sizeof(struct node)))
		{
			printk("Failed to copy_from_user.\n");
			return -1;
		}
