...
//		}
	if(f->fmt.pix.pixelformat == V4L2_PIX_FMT_RGB565){
		...
		*CITRGFMT0 = (3<<29)|(TargetHsize<<16)|(1<<14)|(TargetVsize<<0);
		...
		*CITRGFMT0 = (3<<29)|(TargetHsize<<16)|(1<<14)|(TargetVsize<<0);
		...
		//ycbcr 4:2:2
		*CISCCTRLn &= ~(3<<11);
		*CITRGFMT0 = (2<<29)|(TargetHsize<<16)|(1<<14)|(TargetVsize<<0);
	}
	/*
...
static int ov3640_streamon(struct file *file, void *fh, enum v4l2_buf_type i)
{
	...
	*CISCCTRLn |= (1<<15);//开始缩放
	*CIFCNTSEQn = 0x1;
	printk("sn=%x, s2n=%x, CIFCNTSEQn=%x\n",*CISTATUSn,*CISTATUS2n,*CIFCNTSEQn);
	return 0;
...
static ssize_t ov3640_fops_read(struct file *filp, char __user *buf, size_t size, loff_t *pos)
{
	...
	//for(i=0; i<4; i++){
		ret = copy_to_user(buf,(void *)(buf_addr[0].virt_base),buf_size);
		if(ret){
