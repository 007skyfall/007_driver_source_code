...
int camera_mmap(int fd)
{
	camera_addr = (char *)mmap(0, LENGTH, PROT_READ|PROT_WRITE, MAP_SHARED,fd,0);
	if((int)camera_addr == -1){
		perror("alloc virtual address is fail.");
		return 0;
	}
...
//	read(fd,image_buf,0);
	for(y=0; y<480; y++)
	{
		for(x=0; x<640; x++)
		{
			...
			new_rgb.RGB_h = camera_addr[i];
			new_rgb.RGB_l = camera_addr[i+1];
