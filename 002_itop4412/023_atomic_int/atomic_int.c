...
//定义原子变量,并初始化为0
static atomic_t atomic_value = ATOMIC_INIT(0);

static int atomic_int_open(struct inode *inode, struct file *file){
    printk(KERN_EMERG "atomic_int open!\n");    
    
    if(atomic_read(&atomic_value)){
        return -EBUSY;
    }
...
static int __init atomic_int_init(void)
{
    ...
    printk(KERN_EMERG "atomic_int_init enter!\n");
...
{
    printk(KERN_EMERG "atomic_int_exit!\n");
