...
#define DEVICE_NAME "test_read_gpio_ctl"
...
static long read_gpio_ioctl( struct file *files, unsigned int cmd, unsigned long arg){
	...
	if(arg > 1){
		printk(KERN_EMERG "arg is only 0\n");
	}
...
static int __init read_gpio_init(void)
{
	int DriverState;
	printk("%s,%d\n",__func__,__LINE__);
	printk(KERN_EMERG "read_gpio_init enter!\n");
...
{
	printk(KERN_EMERG "read_gpio_exit exit!\n");
